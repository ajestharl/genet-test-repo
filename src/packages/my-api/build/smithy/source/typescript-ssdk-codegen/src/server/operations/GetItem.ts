...
export type GetItem<Context> = __Operation<
  GetItemServerInput,
  GetItemServerOutput,
  Context
>;
...
export namespace GetItemServerInput {
  ...
  export const validate: (
    obj: Parameters<typeof GetItemInput.validate>[0],
  ) => __ValidationFailure[] = GetItemInput.validate;
}
export interface GetItemServerOutput extends GetItemOutput {}

export type GetItemErrors = ValidationException;

export class GetItemSerializer
  implements
    __OperationSerializer<ExampleServiceService<any>, "GetItem", GetItemErrors>
{
...
export const getGetItemHandler = <Context>(
  operation: __Operation<GetItemServerInput, GetItemServerOutput, Context>,
  customizer: __ValidationCustomizer<"GetItem">,
  ...
  return new GetItemHandler(
    operation,
    mux,
    new GetItemSerializer(),
    serializeFrameworkException,
    customizer,
  );
};
...
export class GetItemHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(
    ...
    if (target === undefined) {
      console.log(
        "Received a request that did not match com.example#ExampleService.GetItem. This indicates a misconfiguration.",
      );
