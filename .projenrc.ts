  ...
  central.addJobs({
    ...
    npm_release: {
      ...
      steps: [
        ...
        {
          name: "Patch projen packages",
          run: [
            'version="${{ needs.bump_version.outputs.version }}"',
            "packages=(ajithapackage ajithapackage2)",
            'for pkg in "${packages[@]}"; do',
        ...
        {
          name: "Patch Smithy packages",
          run: [
            'version="${{ needs.bump_version.outputs.version }}"',
            "declare -A paths=(",
            '  [smithy-client]="src/packages/my-api/build/smithy/source/typescript-client-codegen"',
            '  [smithy-ssdk]="src/packages/my-api/build/smithy/source/typescript-ssdk-codegen"',
            ")",
            'for pkg in "${!paths[@]}"; do',
            '  path="${paths[$pkg]}"',
            '  echo "Patching version in $path/package.json"',
            '  cd "$path"',
            "  jq --arg ver \"$version\" '.version = $ver' package.json > tmp.json && mv tmp.json package.json",
            "  cat package.json | grep version",
            "  cd - > /dev/null",
