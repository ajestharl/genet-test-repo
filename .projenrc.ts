      ...
      steps: [
        ...
        // },
        {
          ...
          run: [
            ...
            "NEXT_VERSION=$(echo $CURRENT_VERSION | awk -F. '{$NF = $NF + 1;} 1' | sed 's/ /./g')",
            'TAG="v$NEXT_VERSION"',
            // Check if tag exists and if it belongs to this package
            "if git ls-remote -q --exit-code --tags origin $TAG >/dev/null 2>&1; then",
            "  TAG_COMMIT=$(git rev-list -n 1 $TAG 2>/dev/null || echo '')",
            '  if [ ! -z "$TAG_COMMIT" ]; then',
            "    # Check if the commit that created this tag modified this package",
            '    if git diff-tree --no-commit-id --name-only -r $TAG_COMMIT | grep -q "$PACKAGE_DIR"; then',
            '      echo "Tag $TAG exists and belongs to this package"',
            '      echo "has_changes=false" >> $GITHUB_OUTPUT',
            "    else",
            '      echo "Tag $TAG exists but belongs to different package"',
            '      echo "version=$NEXT_VERSION" >> $GITHUB_OUTPUT',
            '      echo "has_changes=true" >> $GITHUB_OUTPUT',
            '      echo "v$NEXT_VERSION" > dist/releasetag.txt',
            "    fi",
            "  else",
            '    echo "Tag exists but cannot determine ownership"',
            '    echo "version=$NEXT_VERSION" >> $GITHUB_OUTPUT',
            '    echo "has_changes=true" >> $GITHUB_OUTPUT',
            '    echo "v$NEXT_VERSION" > dist/releasetag.txt',
            "  fi",
